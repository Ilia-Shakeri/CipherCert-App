...
export function HistoryPage({ isDark }: HistoryPageProps) {
  ...
  const [history, setHistory] = useState<HistoryItem[]>([]); // حالت اولیه خالی
  const [loading, setLoading] = useState(false);

  // --- تابع دریافت اطلاعات از بک‌اند ---
  ...
  // وقتی صفحه لود میشه، اطلاعات رو بگیر
  useEffect(() => {
    fetchHistory();
  }, []);
  ...
  // ... (بقیه توابع اکسپورت و فیلتر سر جای خودشون باشن)
    ...
    <div className="p-8 space-y-8">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex gap-3">
          ...
          {/* دکمه‌های اکسپورت رو فعلا غیرفعال یا دکوراتیو نگه دار */}
        </div>
      </div>

      {/* Search Bar */}
      <div className="flex items-center gap-3 px-6 py-4 rounded-2xl border" style={{ background: isDark ? "rgba(15, 23, 42, 0.5)" : "rgba(255, 255, 255, 0.5)", backdropFilter: "blur(20px)", borderColor: isDark ? "rgba(34, 211, 238, 0.2)" : "rgba(8, 145, 178, 0.2)" }}>
          ...
          <table className="w-full">
            <thead>
              <tr style={{ backgroundColor: isDark ? "rgba(15, 23, 42, 0.8)" : "rgba(248, 250, 252, 0.8)", borderBottom: isDark ? "1px solid #1E293B" : "1px solid #E2E8F0" }}>
                <th className="px-6 py-4 text-left text-sm font-semibold" style={{ color: isDark ? "#94A3B8" : "#64748B" }}>Timestamp</th>
                <th className="px-6 py-4 text-left text-sm font-semibold" style={{ color: isDark ? "#94A3B8" : "#64748B" }}>Domain</th>
                ...
                filteredHistory.map((item, index) => (
                  <tr key={item.id || index} className="transition-all duration-200 hover:bg-opacity-50" style={{ backgroundColor: index % 2 === 0 ? (isDark ? "rgba(15, 23, 42, 0.3)" : "rgba(248, 250, 252, 0.3)") : "transparent", borderBottom: isDark ? "1px solid #1E293B" : "1px solid #E2E8F0" }}>
                    ...
                    <td className="px-6 py-4">
                      <span className="px-3 py-1 rounded-lg font-bold" style={{ backgroundColor: (item.grade || "").includes("A") ? "rgba(34, 211, 238, 0.2)" : "rgba(239, 68, 68, 0.2)", color: (item.grade || "").includes("A") ? "#22D3EE" : "#EF4444" }}>
                        {item.grade}
                      </span>
                    </td>
                    <td className="px-6 py-4" style={{ color: isDark ? "#94A3B8" : "#64748B" }}>{item.issuer}</td>
                    <td className="px-6 py-4" style={{ color: isDark ? "#94A3B8" : "#64748B" }}>{item.expiryDate}</td>
                  </tr>
                ))
              )}
            </tbody>
